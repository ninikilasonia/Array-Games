...
public class GamesWithArrays {
  ...
  public static int[] isMore(int[] arr, int index, int x) {
    if (index >= arr.length - 1)
      ...
      return isMore(arr, index + 1, x);
    else if (arr[index] == arr[index + 1])
      return new int[] { -1, index };
    else
      return new int[] { x + 1, index };
  }
  ...
  public static int pinguFriends(int[] arr) {
    ...
    for (int i = 0; i < len - 1; i++) {
      if ((resArr[i] % resArr[i + 1] == 0 || resArr[i + 1] % resArr[i] == 0) && resArr[i] != resArr[i + 1])
        return Math.min(resArr[i], resArr[i + 1]);
